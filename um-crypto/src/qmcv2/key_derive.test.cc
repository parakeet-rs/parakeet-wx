...
TEST(umc_qmcv2_key_derive, make_simple_key) {
  uint8_t buf[8];
  umc::qmc::v2::key_derive::make_simple_key(buf, 106, 8);
...
TEST(umc_qmcv2_key_derive, derive_tea_key) {
  ...
  uint8_t tea_key[16];
  umc::qmc::v2::key_derive::derive_tea_key(tea_key, ekey);
...
TEST(umc_qmcv2_key_derive, parse_ekey_b64) {
  ...
  uint8_t* decoded;
  size_t decode_len;
  umc::qmc::v2::key_derive::from_ekey_b64(decoded, decode_len, ekey);
