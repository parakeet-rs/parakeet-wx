...
std::unique_ptr<XiamiXMConfig> XiamiFileParser::ParseXiamiXMFile(
  ...
  if (ReadBigEndian<u32>(&header[0x00]) != kXiamiHeaderMagic1 ||
      ReadBigEndian<u32>(&header[0x08]) != kXiamiHeaderMagic2) {
    return nullptr;
  }
  ...
  config->noop_len = ReadLittleEndian<u32>(&header[0x0c]) & 0x00ffffff;
