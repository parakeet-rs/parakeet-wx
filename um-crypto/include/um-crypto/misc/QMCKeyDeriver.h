...
class QMCKeyDeriver {
 public:
  virtual bool FromEKey(std::vector<u8>& out,
                        const std::string ekey_b64) const = 0;
  virtual bool FromEKey(std::vector<u8>& out,
                        const std::vector<u8> ekey) const = 0;
  virtual bool ToEKey(std::vector<u8>& out,
                      const std::vector<u8> key) const = 0;
