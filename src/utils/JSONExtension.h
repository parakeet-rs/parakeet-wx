...
#define __PARAKEET_JSON_SET_OR_ADD_MEMBER(value) \
...
inline void WriteValue(D& d, V& v, const char* key, const uint8_t& value) {
  __PARAKEET_JSON_SET_OR_ADD_MEMBER(V(int(value)));
}
#pragma endregion
...
inline void ReadValue(const V& v, const char* key, std::vector<uint8_t>& out, const std::vector<uint8_t>& def_value) {
  if (v.HasMember(key) && v[key].IsString()) {
    out = parakeet_crypto::utils::Base64Decode(std::string(v[key].GetString()));
  } else {
    out = def_value;
  }
}
...
inline std::string ContainerToBase64(const TContainer& in) {
  ...
  for (auto it = in.begin(); it < in.end(); it++) {
    parakeet_crypto::WriteLittleEndian<A>(p_out, *it);
    p_out += sizeof(A);
  }
  return parakeet_crypto::utils::Base64Encode(buf);
}

template <std::unsigned_integral A, class TContainer>
inline void Base64ToContainer(TContainer& out, const std::string& in) {
  std::vector<uint8_t> v_in = parakeet_crypto::utils::Base64Decode(in);
  ...
  for (; it_src <= end_str && it_dst < out.end(); it_src += sizeof(A)) {
    *it_dst++ = parakeet_crypto::ReadLittleEndian<A>(&(*it_src));
  }
}
...
#undef __PARAKEET_JSON_SET_OR_ADD_MEMBER
